...
final public class StringUnionOperations {
	 ...
	 */
	private static testbooks.automaton.State convert(State s,
                                                     IdentityHashMap<State, testbooks.automaton.State> visited) {
		testbooks.automaton.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new testbooks.automaton.State();
	 ...
	 */
	public static testbooks.automaton.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, testbooks.automaton.State>());
	}
