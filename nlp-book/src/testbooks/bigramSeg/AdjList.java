...
public class AdjList implements Iterable<Node> {
	public NodeLinkedList list[];// AdjList的图的结构
	 ...
	 * @return
	 */
	public Iterator<Node> getSuc(int start) {
		NodeLinkedList ll = list[start];
		if (ll == null)
			return null;
		return ll.iterator();
	}
	...
	public NodeLinkedList edges(int pos) {
		if (pos >= list.length) {
			return null;
		}
		return list[pos];
