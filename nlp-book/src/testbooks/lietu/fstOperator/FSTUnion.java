package testbooks.lietu.fstOperator;
...
import testbooks.lietu.fst.CharSpan;
import testbooks.lietu.fst.FST;
import testbooks.lietu.fst.State;
...
public class FSTUnion {
	...
	// 对两个输入集合求并集
	private static Set<CharSpan> union(Set<testbooks.lietu.fst.Transition> setA,
			Set<testbooks.lietu.fst.Transition> setB) {
		Set<CharSpan> tmp = new HashSet<CharSpan>();
		//if (setA != null) {
			for (testbooks.lietu.fst.Transition x : setA) {
				tmp.add(new CharSpan(x.min,x.max));
			}
		//}
		//if (setB != null) {
			for (testbooks.lietu.fst.Transition x : setB) {
				tmp.add(new CharSpan(x.min,x.max));
			}
		//}
		return tmp;
	...
	public static Set<CharSpan> getCharSpans(Set<testbooks.lietu.fst.Transition> transitions){
		HashSet<CharSpan> charSpans = new HashSet<CharSpan>();
		for(testbooks.lietu.fst.Transition t :transitions){
