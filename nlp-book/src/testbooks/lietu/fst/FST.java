...
import testbooks.lietu.fstOperator.FSTStatePair;
import testbooks.lietu.fstOperator.FSTUnion;
import testbooks.lietu.fstOperator.StatePair;

import testbooks.automaton.Automaton;
...
public class FST {
	...
	public FST(Automaton a, String type) throws Exception {
		...
		HashMap<testbooks.automaton.State, State> m = new HashMap<testbooks.automaton.State, State>();
		Set<testbooks.automaton.State> states = a.getStates();
		for (testbooks.automaton.State s : states)
			m.put(s, new State());
		for (testbooks.automaton.State s : states) {
			State p = m.get(s);
			...
			for (testbooks.automaton.Transition t : s.transitions) {
	...
	public static Set<CharSpan> getCharSpans(
			Set<testbooks.lietu.fstOperator.Transition> transitions) {
		HashSet<CharSpan> charSpans = new HashSet<CharSpan>();
		for (testbooks.lietu.fstOperator.Transition t : transitions) {
	...
	public FST(FSTStatePair fstPair) {
		...
		for (StatePair s : states) {
			...
			if (s.transitions == null)
				continue;
			for (testbooks.lietu.fstOperator.Transition t : s.transitions) {
				p.transitions.add(new Transition(t.min, t.max, m.get(t.to)));
			}
		}
	}
